# TODO - Pendientes

## Revisar reglas creadas

> NOTE: Estas reglas fueron creadas recently y necesitan ser revisadas y validadas antes de usarlas en produccion.

### Reglas por revisar

- [ ] **OBSERVABILITY.md** - Logging, Sentry, Prometheus
- [ ] **ONBOARDING.md** - Setup de nuevos miembros
- [ ] **RELEASE.md** - Proceso de lanzamiento
- [ ] **ESTIMATIONS.md** - Guia de estimaciones
- [ ] **DOCKER.md** - Containerizacion
- [ ] **CI_CD.md** - Pipelines
- [ ] **SECURITY.md** - Seguridad
- [ ] **API_DESIGN.md** - Diseno de APIs
- [ ] **DATABASE.md** - Base de datos
- [ ] **EVENT_DRIVEN.md** - Arquitectura de eventos
- [ ] **CODE_REVIEW.md** - Revision de codigo
- [ ] **TROUBLESHOOTING.md** - Problemas comunes
- [ ] **CACHE.md** - Redis cache
- [ ] **CONFIGURATION.md** - pydantic-settings
- [ ] **KANBAN.md** - Kanban boards
- [ ] **IDEATION.md** - Generacion de ideas

## Pendientes

- [ ] Revisar OBSERVABILITY
- [ ] Revisar ONBOARDING
- [ ] Revisar RELEASE
- [ ] Revisar ESTIMATIONS
- [ ] Revisar DOCKER
- [ ] Revisar CI_CD
- [ ] Revisar SECURITY
- [ ] Revisar API_DESIGN
- [ ] Revisar DATABASE
- [ ] Revisar EVENT_DRIVEN
- [ ] Revisar CODE_REVIEW
- [ ] Revisar TROUBLESHOOTING
- [ ] Revisar CACHE
- [ ] Revisar CONFIGURATION

---

*Ultima actualizacion: 2026-03-03*
